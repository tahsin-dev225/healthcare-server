...
const cancelUnpaidAppointments = async () => {
  ...
  const unpainAppointments = await prisma.appointment.findMany({
    where: {
      // status : AppointmentStatus.SCHEDULED,
      createdAt: {
  ...
  const appointmentToCancel = unpainAppointments.map(appointment => appointment.id);
  ...
  await prisma.$transaction(async (tx) => {
    ...
    for (const unpaidAppointment of unpainAppointments) {
      await tx.doctorSchedules.update({
        where: {
          doctorId_scheduleId: {
